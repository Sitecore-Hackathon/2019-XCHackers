...
$addHelixModuleConfigFile = "helixmodule.json"                      # Used in Add-Module.
...
function Create-Config
{
    ...
    if ($jsonFile)
    {
        ...
        Add-Member -InputObject $config -Name FolderNamespace -Value $jsonFile.config.folderNamespace -MemberType NoteProperty
        ...
        Add-Member -InputObject $config -Name TemplateVariableNamePrefix -Value $jsonFile.config.templateVariableNamePrefix -MemberType NoteProperty
        ...
        Add-Member -InputObject $config -Name TemplateFolderGuid -Value $jsonFile.config.templateFolderGuid -MemberType NoteProperty
        Add-Member -InputObject $config -Name RenderingFolderGuid -Value $jsonFile.config.renderingFolderGuid -MemberType NoteProperty
        Add-Member -InputObject $config -Name TemplateParentFolderGuid -Value $jsonFile.config.templateParentFolderGuid -MemberType NoteProperty
        Add-Member -InputObject $config -Name RenderingParentFolderGuid -Value $jsonFile.config.renderingParentFolderGuid -MemberType NoteProperty
        ...
        # GUIDs are needed for .yml files
        $templateFolderGuid = [guid]::NewGuid().toString().toLower()
        Add-Member -InputObject $config -Name TemplateFolderGuidValue -Value $templateFolderGuid -MemberType NoteProperty
        $renderingFolderGuid = [guid]::NewGuid().toString().toLower()
        Add-Member -InputObject $config -Name RenderingFolderGuidValue -Value $renderingFolderGuid -MemberType NoteProperty
        ...
        $newNamespacePrefix = ""
        $newTemplateParentFolderGuid = ""
        $newRenderingParentFolderGuid = ""
        
		if ($ModuleType -eq $featureModuleType)
        {
            $newNamespacePrefix = $jsonFile.config.featureNamespacePrefix
			$newTemplateParentFolderGuid = $jsonFile.config.templateFeatureParentFolderGuid
			$newRenderingParentFolderGuid = $jsonFile.config.renderingFeatureParentFolderGuid
        }
        if ($ModuleType -eq $foundationModuleType)
        {
            $newNamespacePrefix = $jsonFile.config.foundationNamespacePrefix
			$newTemplateParentFolderGuid = $jsonFile.config.templateFoundationParentFolderGuid
			$newRenderingParentFolderGuid = $jsonFile.config.renderingFoundationParentFolderGuid
        }

        Add-Member -InputObject $config -Name NamespacePrefix -Value $newNamespacePrefix -MemberType NoteProperty
        Add-Member -InputObject $config -Name FolderName -Value $newNamespacePrefix.Replace(".", "-") -MemberType NoteProperty
        Add-Member -InputObject $config -Name VariableNamePrefix -Value ($config.ModuleName.substring(0,1).tolower() + $config.ModuleName.substring(1)) -MemberType NoteProperty
        Add-Member -InputObject $config -Name TemplateParentFolderGuidValue -Value $newTemplateParentFolderGuid -MemberType NoteProperty
        Add-Member -InputObject $config -Name RenderingParentFolderGuidValue -Value $newRenderingParentFolderGuid -MemberType NoteProperty
...
function Rename-Module
{
    ...
    Rename-Folders -StartPath "$StartPath" -OldValue $config.FolderNamespace -NewValue $config.FolderName
    ...
    # Update .yml file content for GUIDs.
    Update-FileContent -StartPath "$StartPath" -OldValue $config.TemplateFolderGuid -NewValue $config.TemplateFolderGuidValue -FileExtensionsRegex $config.FileExtensionsToUpdateContentRegex
    Update-FileContent -StartPath "$StartPath" -OldValue $config.RenderingFolderGuid -NewValue $config.RenderingFolderGuidValue -FileExtensionsRegex $config.FileExtensionsToUpdateContentRegex
    Update-FileContent -StartPath "$StartPath" -OldValue $config.TemplateParentFolderGuid -NewValue $config.TemplateParentFolderGuidValue -FileExtensionsRegex $config.FileExtensionsToUpdateContentRegex
    Update-FileContent -StartPath "$StartPath" -OldValue $config.RenderingParentFolderGuid -NewValue $config.RenderingParentFolderGuidValue -FileExtensionsRegex $config.FileExtensionsToUpdateContentRegex
    ...
    Update-FileContent -StartPath "$StartPath" -OldValue $config.FolderNamespace -NewValue $config.FolderName -FileExtensionsRegex $config.FileExtensionsToUpdateContentRegex
    ...
    Update-FileContent -StartPath "$StartPath" -OldValue $config.TemplateVariableNamePrefix -NewValue $config.VariableNamePrefix -FileExtensionsRegex $config.FileExtensionsToUpdateContentRegex
}
...
function Add-Module
{
    ...
    try
    {
        ...
        $copyModuleFromLocation = Join-Path -Path (Convert-Path (Get-Location -PSProvider FileSystem)) -ChildPath (Join-Path -Path $config.ModuleTemplatePath -ChildPath $config.TemplateModuleName)
        Write-Output $copyModuleFromLocation
